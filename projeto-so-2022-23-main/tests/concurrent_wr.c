#include "../fs/operations.h"
#include <assert.h>
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
//apagar e do mendo e daniel
#define NUM_THREADS 23 // max dir entries in a directory with block size 1024
char const file_contents[] = "message";

typedef struct {
    char* path_generated_file;
} thread_input;

void* thread_fn(void* in) {
    thread_input* args = (thread_input*)in;
    int fh = tfs_open(args->path_generated_file, TFS_O_CREAT);
    assert(fh != 1);
    assert(tfs_write(fh, file_contents, strlen(file_contents)) != -1);
    assert(tfs_close(fh) != -1);
    return NULL;
}

int main() {
    pthread_t tid[NUM_THREADS];

    tfs_params params = tfs_default_params();

    assert(tfs_init(&params) != -1);

    for (int i = 0; i < NUM_THREADS; i++) {
        thread_input* input = (thread_input*)malloc(sizeof(thread_input));
        input->path_generated_file =
            malloc(strlen(file_contents) * sizeof(char));
        sprintf(input->path_generated_file, "/f%d", i);
        if (pthread_create(&tid[i], NULL, thread_fn, (void*)input) != 0)
            return 1;
    }

    for (int i = 0; i < NUM_THREADS; i++) {
        pthread_join(tid[i], NULL);
    }

    assert(tfs_open("/overTheLimit", TFS_O_CREAT) == -1);

    for (int i = 0; i < NUM_THREADS; i++) {
        char file_name[7];
        char buffer[7];
        sprintf(file_name, "/f%d", i);
        int lh = tfs_open(file_name, 0);
        assert(lh != -1);
        assert(tfs_read(lh, buffer, sizeof(buffer)) == strlen(file_contents));
        assert(memcmp(buffer,file_contents,sizeof(buffer)) == 0);
        assert(tfs_close(lh) != -1);
    }
    printf("\033[92m Successful test.\n\033[0m");

    assert(tfs_destroy() != -1);
    return 0;
}
